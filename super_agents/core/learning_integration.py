...
class LearningIntegration:
    ...
    def post_execute(
        ...
        """
        if not self.agent_support:
            return {"success": False, "message": "Learning system unavailable"}

        try:
            return self.agent_support.end_execution_and_learn(
                agent_id, status=status, result=result
            )
        except Exception as e:
...
def execute_with_learning(
    ...
    learning_result = learning.post_execute(
        agent_id, status=status, result={"task": task_description, "status": status}
    )
