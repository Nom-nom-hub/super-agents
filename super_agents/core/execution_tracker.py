...
class ExecutionTracker:
    ...
    def end_execution(
        self, status: str = "completed", result: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        """
        if not hasattr(self, "current_execution"):
            self.logger.warning("No current execution to end")
            return {}
    ...
    def get_aggregate_stats(self, agent_id: str) -> Dict:
        ...
        stats = {
            ...
            "avg_latency_ms": self._get_avg_metric(
                executions, "performance_latency_ms"
            ),
    ...
    def export_for_reflection(self, agent_id: str) -> Dict:
        ...
        return {
            ...
            "success_rate": (
                len([e for e in executions if e["status"] == "completed"])
                / len(executions)
                if executions
                else 0
            ),
