...
class AgentSpecValidator:
    ...
    def __init__(self, schema_path: str = None):
        ...
        if schema_path is None:
            ...
            schema_path = os.path.join(current_dir, "..", "schemas", "agent_spec_schema.json")
    ...
    def _custom_validations(self, spec: Dict[str, Any]) -> List[str]:
        ...
        if agent_id:
            ...
            if not self._is_valid_id_format(agent_id):
                errors.append(f"Invalid ID format: '{agent_id}'. Should be snake_case (lowercase with underscores).")
    ...
    def _check_duplicates(self, items: List[str], field_name: str) -> List[str]:
        ...
        for item in items:
            ...
            if item_lower in seen:
                errors.append(f"Duplicate {field_name}: '{item}' appears multiple times (case-insensitive)")
            seen.add(item_lower)
    ...
    def validate_all_agent_specs(self, agents_dir: str) -> Dict[str, Tuple[bool, List[str]]]:
        """
