...
class ReflectionAgent:
    ...
    def _extract_patterns(self, executions: List[Dict]) -> List[Dict]:
        ...
        for execution in executions:
            if execution["status"] == "completed":
                decisions = tuple(
                    sorted([d["decision"] for d in execution.get("decisions_made", [])])
                )
                if decisions:
                    decision_combinations[decisions] = (
                        decision_combinations.get(decisions, 0) + 1
                    )
            ...
            recommendations.append(
                {
                    ...
                    "severity": (
                        "critical"
                        if percentage > 50
                        else "high" if percentage > 25 else "medium"
                    ),
            ...
            if execution["status"] == "completed":
                ...
                for decision in execution.get("decisions_made", []):
                    ...
                    if decision_text:
                        ...
                        capability = self._extract_capability_from_decision(
                            decision_text
                        )
                        if capability:
    ...
    def _detect_specialization(self, executions: List[Dict]) -> Optional[str]:
        ...
        common_keywords = [
            kw for kw, count in keyword_counts.most_common(5) if count > 1
        ]
    ...
    def _extract_performance_baseline(self, stats: Dict) -> Dict:
        ...
        return {
            ...
            "avg_test_coverage": (
                round(stats.get("avg_test_coverage", 0), 1)
                if stats.get("avg_test_coverage")
                else None
            ),
            "avg_code_quality": (
                round(stats.get("avg_code_quality", 0), 1)
                if stats.get("avg_code_quality")
                else None
            ),
            "avg_latency_ms": (
                round(stats.get("avg_latency_ms", 0), 1)
    ...
    def _extract_quality_metrics(self, stats: Dict) -> Dict:
        ...
        return {
            ...
            "avg_lines_per_execution": (
                round(stats.get("total_lines_of_code", 0) / total, 0)
                if total > 0
                else 0
            ),
        }

    def generate_spec_update(self, current_spec: Dict, learnings: Dict) -> Dict:
        """
        ...
        if learnings.get("tools_discovered"):
            ...
            updated_spec["tools"] = sorted(current_tools | new_tools)
        ...
        if learnings.get("proven_capabilities"):
            ...
            updated_spec["capabilities"] = sorted(current_capabilities | proven)
        ...
        if learnings.get("patterns_emerged"):
            updated_spec["patterns_learned"] = [
                p["pattern"] for p in learnings["patterns_emerged"]
            ]
    ...
    def validate_spec_changes(self, old_spec: Dict, new_spec: Dict) -> Dict:
        """
        ...
        changes = {
            "tools_added": list(
                set(new_spec.get("tools", [])) - set(old_spec.get("tools", []))
            ),
            "capabilities_added": list(
                set(new_spec.get("capabilities", []))
                - set(old_spec.get("capabilities", []))
            ),
            "version_increment": new_spec.get("version", 1)
            - old_spec.get("version", 1),
...
"""
        ...
        report += (
            f"- Requires Review: {'Yes' if changes['requires_review'] else 'No'}\n"
        )
