...
class AutonomousSpecManager:
    ...
    def version_spec(self, agent_id: str, spec: Dict) -> str:
        ...
        versioned_path = os.path.join(
            self.history_dir,
            f"{agent_id}_agent_{version_str}.yaml"
        )
    ...
    def regenerate_spec(
        ...
        if change_validation["requires_review"]:
            governance_note = (
                f"⚠ Review Required: {change_validation['review_reason']}"
            )
    ...
    def _validate_spec_schema(self, spec: Dict) -> bool:
        ...
        for field, expected_type in optional_validations.items():
            if field in spec and not isinstance(spec[field], expected_type):
                print(f"❌ Schema validation failed: '{field}' must be {expected_type.__name__}")
                return False
    ...
    def _update_intent_mappings(self, agent_id: str, learnings: Dict):
        ...
        sub_spec_id = f"{agent_id}_{specialization.lower().replace(' ', '_')}"
        ...
        for intent in intent_mapping.get("intents", []):
            if agent_id in intent.get("primary_agent", "") or agent_id in intent.get(
                ...
                if not any(
                    s.get("agent") == agent_id and s.get("specialization") == specialization
    ...
    def get_spec_history(self, agent_id: str) -> list:
        ...
        for filename in sorted(os.listdir(self.history_dir)):
            if filename.startswith(f"{agent_id}_agent_v") and filename.endswith(".yaml"):
    ...
    def compare_specs(self, agent_id: str, version1: int, version2: int) -> Dict:
        ...
        def load_version(version):
            path = os.path.join(
                self.history_dir,
                f"{agent_id}_agent_v{version}.yaml"
            )
        ...
        return {
            ...
            "capabilities_added": list(
                set(spec2.get("capabilities", []))
                - set(spec1.get("capabilities", []))
            ),
    ...
    def rollback_spec(self, agent_id: str, target_version: int) -> bool:
        ...
        version_path = os.path.join(
            self.history_dir,
            f"{agent_id}_agent_v{target_version}.yaml"
        )
    ...
    def get_agent_evolution_summary(self, agent_id: str) -> Dict:
        ...
        summary = {
            ...
            "versions": history,
            "evolution": {
                "tools_growth": len(latest.get("tools", [])) - len(
                    earliest.get("tools", [])
                ),
                "capabilities_growth": len(latest.get("capabilities", [])) - len(
                    earliest.get("capabilities", [])
                ),
                "specialization": latest.get("specialization_area", "None"),
                "quality_improvement": {
                    "success_rate": latest.get("quality_metrics", {}).get("success_rate"),
